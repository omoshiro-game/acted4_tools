...
function requireConfig() {
  if (hasRequiredConfig) return config$2;
  hasRequiredConfig = 1;
...
var src = Encoding;
const Encoding$1 = /* @__PURE__ */ getDefaultExportFromCjs(src);
(() => {
...
function ensureEncodingModule() {
  if (!Encoding$1 || typeof Encoding$1.convert !== "function") {
    throw new Error("encoding-japanese module is required to handle Shift-JIS data.");
  }
  return Encoding$1;
}
function normalizeForEncoding(value) {
  if (!value) {
    return "";
  }
  return value.replace(/\u2212/g, "－");
...
function readVersion(reader) {
  ...
  if (!MAGIC_VALUES.has(version2)) {
    throw new Error(`Unexpected database version: 0x${version2.toString(16)}`);
  }
  return version2;
}
...
function parseList(source, elementFactory) {
  ...
  const version2 = readVersion(reader);
...
function serializeAnime(database) {
  return writeList(database, writeAnimation);
}
function parseAnimeSet(source) {
  const reader = ensureReader(source);
  const version2 = readVersion(reader);
  const count = reader.readUint32();
...
function serializeAnimeSet(database) {
  var _a;
  const writer = new DataWriter();
  writeVersion(writer, database.version);
  const elements = ((_a = database == null ? void 0 : database.data) == null ? void 0 : _a.elements) ?? [];
...
function serializeEffect(database) {
  var _a;
  const writer = new DataWriter();
  writeVersion(writer, database.version);
  const elements = ((_a = database == null ? void 0 : database.data) == null ? void 0 : _a.elements) ?? [];
...
function serializeScrEffect(database) {
  return writeList(database, writeScreenEffectElement);
}
function parseSwordType(source) {
  const reader = ensureReader(source);
  const version2 = readVersion(reader);
  const count = reader.readUint32();
...
function serializeSwordType(database) {
  var _a;
  const writer = new DataWriter();
  writeVersion(writer, database.version);
  const elements = ((_a = database == null ? void 0 : database.data) == null ? void 0 : _a.elements) ?? [];
...
const SERIALIZER_MAP = /* @__PURE__ */ new Map([
